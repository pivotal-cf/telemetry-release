...
describe 'Telemetry Collector Error Handling' do
  ...
  describe 'structured logging' do
    ...
    it 'creates valid JSON log entry for customer config error' do
      ...
      log_entry = create_log_entry(timestamp, 'CUSTOMER_CONFIG_ERROR', 'API key is invalid or expired', exit_code,
                                   error_output)
    ...
    it 'creates valid JSON log entry for middleware pipeline error' do
      ...
      log_entry = create_log_entry(timestamp, 'MIDDLEWARE_PIPELINE_ERROR',
                                   'Telemetry infrastructure is temporarily unavailable', exit_code, error_output)
