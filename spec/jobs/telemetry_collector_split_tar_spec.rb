...
describe 'Telemetry Collector Split TAR Handling' do
  ...
  def classify_error_type(output)
    ...
    when /unauthorized|not authorized|401/
      'CUSTOMER_CONFIG_ERROR'
    when /kinit.*not found|curl.*not found|gss-api/
      'SYSTEM_REQUIREMENTS_ERROR'
    when /connection refused|timeout|503|502|504/
      'MIDDLEWARE_PIPELINE_ERROR'
    when /proxy.*authentication|407|spnego|kerberos/
      'PROXY_AUTH_ERROR'
    else
      'UNKNOWN_ERROR'
    end
  ...
  def simulate_tar_handling(tar_files:, send_exit_codes:, send_outputs: nil, audit_mode: false)
    ...
    if tar_count > 1
      ...
      stderr_output << "WARNING: #{send_failures} of #{tar_count} TAR file(s) failed to send" if send_failures > 0
    end
