...
describe 'Filters telemetry messages' do
  ...
  it 'returns telemetry messages without agent version if not present' do
    ...
    expect(records).to(eq([{ 'data' => { 'app' => 'da"ta', 'counter' => 42, 'array' => [{ 'cool' => 'value' }] },
                             'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
  end

  it 'returns telemetry messages with the agent version merged in' do
    records = filter({ 'agent-version' => '0.0.1',
                       'log' => '{ "telemetry-source": "my-origin", "telemetry-time": "2019-10-23T14:49:39-04:00", "data": {"app": "da\"ta", "counter": 42, "array": [ {"cool":"value"} ]}}' })

    expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1',
                             'data' => { 'app' => 'da"ta', 'counter' => 42, 'array' => [{ 'cool' => 'value' }] }, 'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
  end
  ...
  it 'returns telemetry messages when the telemetry message has object chars in the keys or values after the telemetry-source key' do
    records = filter({ 'agent-version' => '0.0.1',
                       'log' => '{ "telemetry-source": "my-{origin", "telemetry-time": "2019-10-23T14:49:39-04:00", "data": {"}app": "d}ata"}} maybe some junk here' })
    expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1', 'data' => { '}app' => 'd}ata' },
                             'telemetry-source' => 'my-{origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
  end
  ...
  it 'returns telemetry messages when the telemetry message has object chars in the keys or values before the telemetry-source key' do
    records = filter({ 'agent-version' => '0.0.1',
                       'log' => '{ "data": {"}app": "d}ata"}, "telemetry-source": "my-{origin", "telemetry-time": "2019-10-23T14:49:39-04:00"} maybe some junk here' })
    expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1', 'data' => { '}app' => 'd}ata' },
                             'telemetry-source' => 'my-{origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
  end
  ...
  context 'when telemetry messages appears as a JSON object embedded in other log lines' do
    it 'returns messages when nested in another JSON object' do
      records = filter({ 'agent-version' => '0.0.1',
                         'log' => '{ "time": 12341234123412, "level": "info", "message": "whatever", "data": { "something": "else", "telemetry-thing": { "telemetry-source": "my-origin",  "telemetry-time": "2019-10-23T14:49:39-04:00", "data": {"app": "da\"ta", "counter": 42, "array": [ {"cool":"value"} ]}}, "more": "otherthings"}' })
      expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1',
                               'data' => { 'app' => 'da"ta', 'counter' => 42, 'array' => [{ 'cool' => 'value' }] }, 'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'returns messages embedded in an arbitrary string' do
      records = filter({ 'agent-version' => '0.0.1',
                         'log' => 'Tue 14-Mar-2019 [Thread-14] com.java.SillyClass myhostname { "telemetry-source": "my-origin", "telemetry-time": "2019-10-23T14:49:39-04:00", "data": {"app": "data"}} maybe some junk here' })
      expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1', 'data' => { 'app' => 'data' },
                               'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
  end
  ...
  context 'when telemetry messages are embedded as escaped json strings' do
    it 'returns messages' do
      records = filter({ 'agent-version' => '0.0.1',
                         'log' => '{ "time": 12341234123412, "level": "info", "message": "{ \"data\": {\"app\": \"da\\\\\"ta\", \"counter\": 42}, \"telemetry-source\": \"my-origin\", \"telemetry-time\": \"2019-10-23T14:49:39-04:00\"}" }' })
      expect(records).to(eq([{ 'telemetry-agent-version' => '0.0.1', 'data' => { 'app' => 'da"ta', 'counter' => 42 },
                               'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
  ...
  it 'rejects messages with a log key that does not contain RFC3339 formatted "telemetry-time"' do
    ...
    expect(records).to(be_empty)
    expect(driver.logs).to(include(%r{telemetry-time field from event <some log line {"telemetry-source": "some-source", "telemetry-time": "foo"}> must be in date/time format RFC 3339. Cause: .+}))
  end
  ...
  context 'characterization tests for backward compatibility' do
    it 'handles empty string values in JSON' do
      ...
      expect(records).to(eq([{ 'data' => { 'empty' => '' }, 'telemetry-source' => '',
    ...
    it 'handles unicode characters in telemetry data' do
      ...
      expect(records).to(eq([{ 'data' => { 'emoji' => '🔥', 'japanese' => '日本語', 'arabic' => 'العربية' },
                               'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles numeric values correctly' do
      ...
      expect(records).to(eq([{ 'int' => 42, 'float' => 3.14, 'negative' => -10, 'zero' => 0,
                               'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles boolean values correctly' do
      ...
      expect(records).to(eq([{ 'enabled' => true, 'disabled' => false, 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles null values correctly' do
      ...
      expect(records).to(eq([{ 'nullable' => nil, 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles empty arrays and objects' do
      ...
      expect(records).to(eq([{ 'empty_array' => [], 'empty_object' => {}, 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles arrays with mixed types' do
      ...
      expect(records).to(eq([{ 'mixed' => [1, 'string', true, nil, { 'nested' => 'object' }],
                               'telemetry-source' => 'my-origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles different RFC3339 timestamp formats' do
      ...
      expect(records.last).to(eq({ 'data' => 'utc', 'telemetry-source' => 'my-origin',
                                   'telemetry-time' => '2019-10-23T14:49:39Z' }))

      records = filter({ 'log' => '{ "telemetry-source": "my-origin", "telemetry-time": "2019-10-23T14:49:39+00:00", "data": "plus_zero"}' })
      expect(records.last).to(eq({ 'data' => 'plus_zero', 'telemetry-source' => 'my-origin',
                                   'telemetry-time' => '2019-10-23T14:49:39+00:00' }))

      records = filter({ 'log' => '{ "telemetry-source": "my-origin", "telemetry-time": "2019-10-23T14:49:39.123456Z", "data": "microseconds"}' })
      expect(records.last).to(eq({ 'data' => 'microseconds', 'telemetry-source' => 'my-origin',
                                   'telemetry-time' => '2019-10-23T14:49:39.123456Z' }))
    end
    ...
    it 'handles escaped quotes within values' do
      ...
      expect(records).to(eq([{ 'quote' => 'He said "hello"', 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles reasonably sized log lines (documents size handling)' do
      ...
      expect(records).to(eq([{ 'data' => large_value, 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles backslashes at string boundaries correctly' do
      ...
      expect(records).to(eq([{ 'path' => 'C:\\Windows\\', 'telemetry-source' => 'my-origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles messages where telemetry-source appears multiple times (uses first occurrence)' do
      ...
      expect(records.length).to be >= 1
      expect(records.first['telemetry-source']).to eq('origin1') if records.length.positive?
    end
  end
  ...
  context 'edge cases and error handling' do
    ...
    it 'handles telemetry-source with Unicode characters' do
      ...
      expect(records).to(eq([{ 'data' => 'test', 'telemetry-source' => '服务器-🚀',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles multiple consecutive backslashes in different positions' do
      ...
      expect(records).to(eq([{ 'three' => '\\\\\\', 'four' => '\\\\\\\\', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles JSON with whitespace variations (tabs and multiple spaces)' do
      ...
      expect(records).to(eq([{ 'data' => 'value', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles negative timezone offsets in RFC3339' do
      ...
      expect(records).to(eq([{ 'data' => 'test', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-12:00' }]))
    end
    ...
    it 'handles positive timezone offsets in RFC3339' do
      ...
      expect(records).to(eq([{ 'data' => 'test', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39+14:00' }]))
    end
    ...
    it 'handles deeply nested arrays' do
      ...
      expect(records).to(eq([{ 'nested' => [[[[['deep']]]]], 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles scientific notation numbers' do
      ...
      expect(records).to(eq([{ 'sci' => 1.23e10, 'neg' => -4.56e-7, 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles strings with control characters (properly escaped)' do
      ...
      expect(records).to(eq([{ 'ctrl' => "line1\nline2\r\nline3\ttab", 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles forward slashes in strings (no escaping needed in JSON)' do
      ...
      expect(records).to(eq([{ 'url' => 'https://example.com/path/to/resource', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles escaped forward slashes in strings (valid but unnecessary in JSON)' do
      ...
      expect(records).to(eq([{ 'url' => 'https://example.com/path', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles Unicode escape sequences in JSON' do
      ...
      expect(records).to(eq([{ 'unicode' => 'A☺', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles keys with special JSON characters (but properly quoted)' do
      ...
      expect(records).to(eq([{ 'key:with:colons' => 'value', 'key{with}braces' => 'value2',
                               'telemetry-source' => 'origin', 'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'rejects JSON with duplicate keys (documents JSON parser behavior)' do
      ...
      if records.length.positive?
        ...
        expect(%w[first second]).to include(records.first['telemetry-source'])
      end
    end
    ...
    it 'handles agent-version with special characters' do
      records = filter({ 'agent-version' => 'v1.2.3-beta+build.123',
                         'log' => '{ "telemetry-source": "origin", "telemetry-time": "2019-10-23T14:49:39-04:00"}' })
      expect(records).to(eq([{ 'telemetry-agent-version' => 'v1.2.3-beta+build.123', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end

    it 'handles agent-version as empty string' do
      records = filter({ 'agent-version' => '',
                         'log' => '{ "telemetry-source": "origin", "telemetry-time": "2019-10-23T14:49:39-04:00"}' })
      expect(records).to(eq([{ 'telemetry-agent-version' => '', 'telemetry-source' => 'origin',
                               'telemetry-time' => '2019-10-23T14:49:39-04:00' }]))
    end
    ...
    it 'handles very long key names (1000 characters)' do
      ...
      expect(records.first[long_key]).to eq('value') if records.length.positive?
    end
  ...
  context 'malformed JSON handling' do
    ...
    it 'handles escape sequences as literal strings in single-quoted Ruby literals' do
      ...
      expect(records.first['hex']).to eq('x41') if records.length.positive?
    end
    ...
    it 'handles null byte in JSON string (if supported by parser)' do
      ...
      expect(records.length).to be >= 0
      expect(records.first['telemetry-source']).to include('test') if records.length.positive?
    end
  ...
  context 'performance benchmarks (large logs)', :benchmark do
    ...
    rescue Timeout::Error, Fluent::Test::Driver::TestTimedOut
      GC.enable
      {
        description: description,
        size_mb: log_line.bytesize / 1024.0 / 1024.0,
